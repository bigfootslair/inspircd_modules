...
class WebircMode : public ModeHandler
{
public:
	WebircMode(Module *Creator) : ModeHandler(Creator, "webirc", 'q', PARAM_NONE, MODETYPE_USER) { oper = true; }

	ModeAction OnModeChange(User *source, User *dest, Channel *channel, std::string &parameter, bool adding)
	{
		if (adding && IS_SERVER(source)) // User is adding the mode and the server set the mode
		{
			...
			for(CGIHostlist::iterator iter = Hosts.begin(); iter != Hosts.end(); iter++)
			{
				if(InspIRCd::Match(user->host, iter->hostmask, ascii_case_insensitive_map) || InspIRCd::MatchCIDR(user->GetIPString(), iter->hostmask, ascii_case_insensitive_map))
				{
					if(iter->type == WEBIRC && parameters[0] == iter->password)
					{
						...
						user->SetMode('q', true);
						user->WriteServ("MODE " + user->nick + " +q");
 ...
 public:
	...
	virtual void OnLookupComplete(const std::string &result, unsigned int ttl, bool cached)
	{
		...
		if ((them) && (!them->quitting))
		{
			...
			them->SetMode('q', true);
			them->WriteServ("MODE " + them->nick + " +q");
...
public:
	ModuleCgiIRC() : webm(this), cmd(this), waiting("cgiirc-delay", this)
	{
	}
	...
	void init()
	{
		...
		ServerInstance->Modules->AddService(webm);
